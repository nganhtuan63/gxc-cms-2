		...
		if($cache_id){
			switch ($cache_id) {
				case 'frontend_assets':	
					...
					user()->setFlash('success',t('cms','FRONTEND Assets cleared!'));		
					break;
				case 'frontend_cache':	
					if($this->clearCache('frontend'))		
						user()->setFlash('success',t('cms','FRONTEND Cache cleared!'));
					else 
						user()->setFlash('error',t('cms','Error while clear FRONTEND Cache!'));	
					break;
				case 'backend_assets':					
					$this->clearCacheAssets('backend');
					user()->setFlash('success',t('cms','BACKEND Assets cleared!'));
					break;
				case 'backend_cache':	
					if($this->clearCache('backend'))		
						user()->setFlash('success',t('cms','BACKEND Cache cleared!'));
					else		
						user()->setFlash('error',t('cms','Error while clear BACKEND Cache!'));
					break;						
				default:					
					break;
			}
