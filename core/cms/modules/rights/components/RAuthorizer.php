...
class RAuthorizer extends CApplicationComponent
{
	...
	* @return the permission tree.
	*/
	public function getPermissionsWithBizRule($itemName=null)
	{
		$permissions = array();
		...
		{
			$item = $this->_authManager->getAuthItem($itemName);
			$permissions = $this->getPermissionsWithBizRueRecursive($item);
		}
		else
		{
			foreach( $this->getRoles() as $roleName=>$role ){				
				$permissions[ $roleName ]['items'] = $this->getPermissionsWithBizRueRecursive($role);
				$permissions[ $roleName ]['bizRule'] = $role->bizRule;
			}
		}
